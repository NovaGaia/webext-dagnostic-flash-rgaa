...
function testColorOnly() {
  const testId = 'color-only';
  ...
  testItem.innerHTML = `
    <div class="test-name">${t('testColorOnlyName')}</div>
  ...
  validationInputs.forEach(input => {
    input.addEventListener('change', () => {
      updateColorOnlyStatus(testId, input.value);
    });
  });
}

// Mettre à jour le statut du test
function updateColorOnlyStatus(testId, validationValue) {
  ...
  const testData = {
    name: t('testColorOnlyNameForStats'),
    status: status,
